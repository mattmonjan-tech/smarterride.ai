...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin, onQuoteRequest }) => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 relative">
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8 z-10">
            ...
            <div className="pt-8 flex items-center gap-6 text-sm text-slate-500 font-medium">
              ...
              <span className="flex items-center gap-2 px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs font-bold">v2.0 Live</span>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-8">
            <div className="col-span-1 md:col-span-2">
                <div className="flex items-center gap-2 mb-4">
                    <div className="bg-blue-600 p-1.5 rounded text-white"><Bus size={20} /></div>
                    <span className="text-lg font-bold">RideSmart.ai</span>
                </div>
                <p className="text-slate-400 max-w-xs text-sm leading-relaxed">Empowering school districts with next-generation logistics and safety tools.</p>
            </div>
            <div>
      ...
      {/* Modals (Login, Quote, Hardware, PO, Email Preview) */}
              ...
              <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-300">
                  ...
                  <div className="p-8 bg-white">
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-100 flex items-center gap-3">
                           <div className="h-12 w-12 bg-red-100 text-red-600 rounded-lg flex items-center justify-center"><FileText size={24} /></div>
                           <div><p className="text-sm font-bold text-slate-800">Quote-{generatedQuote.id}.pdf</p><p className="text-xs text-slate-400">145 KB</p></div>
                      </div>
                  </div>
              </div>
          </div>
                      ...
                      {!generatedQuote ? (
                          <form onSubmit={handleQuoteSubmit} className="space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                  <div><label className="block text-sm font-bold text-slate-700 mb-1">Contact Name</label><input required type="text" className="w-full border border-slate-300 rounded-lg p-2 text-sm" value={quoteForm.contact} onChange={e => setQuoteForm({...quoteForm, contact: e.target.value})} /></div>
                                  <div><label className="block text-sm font-bold text-slate-700 mb-1">Role</label><select required className="w-full border border-slate-300 rounded-lg p-2 text-sm bg-white" value={quoteForm.role} onChange={e => setQuoteForm({...quoteForm, role: e.target.value})}><option value="">Select...</option><option value="Superintendent">Superintendent</option><option value="Transportation Director">Transportation Director</option><option value="IT Director">IT Director</option><option value="Business Manager">Business Manager</option><option value="Other">Other</option></select></div>
                              </div>
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                  <div><label className="block text-sm font-bold text-slate-700 mb-1">Total Students</label><input required type="number" className="w-full border border-slate-300 rounded-lg p-2 text-sm" value={quoteForm.students} onChange={e => setQuoteForm({...quoteForm, students: e.target.value})} /></div>
                                  <div><label className="block text-sm font-bold text-slate-700 mb-1">Total Buses</label><input required type="number" className="w-full border border-slate-300 rounded-lg p-2 text-sm" value={quoteForm.buses} onChange={e => setQuoteForm({...quoteForm, buses: e.target.value})} /></div>
                              </div>
                          ...
                          <div className="animate-in fade-in slide-in-from-bottom-4">
                              <div id="quote-document" className="bg-white p-8 border border-slate-200 shadow-sm mb-6 font-poppins text-slate-800">
                                  <div className="flex justify-between items-start mb-8 border-b-2 border-slate-900 pb-6">
                                      <div><div className="flex items-center gap-2 text-blue-600 mb-2"><Bus size={24} /><span className="text-xl font-bold text-slate-900">RideSmart.ai</span></div><p className="text-xs text-slate-500">123 Innovation Drive<br/>Tech Valley, CA 94043</p></div>
                                      <div className="text-right"><h2 className="text-2xl font-bold text-slate-900">QUOTE</h2><p className="text-sm text-slate-500">#{generatedQuote.id}</p><p className="text-sm text-slate-500">Date: {generatedQuote.submittedDate}</p></div>
                                  </div>
                                  ...
                                  <table className="w-full mb-8">
                                      <thead><tr className="border-b border-slate-200 text-left text-xs font-bold text-slate-500 uppercase"><th className="py-2">Description</th><th className="py-2 text-right">Qty</th><th className="py-2 text-right">Total</th></tr></thead>
                                      <tbody className="text-sm">
                                          <tr className="border-b border-slate-100"><td className="py-4"><p className="font-bold text-slate-900">{generatedQuote.tier} Subscription</p><p className="text-xs text-slate-500">Annual Platform License & Support</p></td><td className="py-4 text-right">1</td><td className="py-4 text-right font-mono">${(generatedQuote.amount - hardwareCost).toLocaleString()}</td></tr>
                                          {generatedQuote.legacyBusCount && generatedQuote.legacyBusCount > 0 && (<tr className="border-b border-slate-100"><td className="py-4"><p className="font-bold text-slate-900">RideSmart Retrofit Kit</p><p className="text-xs text-slate-500">Hardware for pre-2015 vehicles (USB/Dongle)</p></td><td className="py-4 text-right">{generatedQuote.legacyBusCount}</td><td className="py-4 text-right font-mono">${hardwareCost.toLocaleString()}</td></tr>)}
                                          {discountDetails.perBus > 0 && (<tr className="border-b border-slate-100 bg-green-50/50"><td className="py-4 pl-2"><p className="font-bold text-green-700">Volume Discount Applied</p><p className="text-xs text-green-600">Tiered savings for fleet > 100 buses</p></td><td className="py-4 text-right"></td><td className="py-4 text-right font-mono text-green-700">-${discountDetails.totalDiscount.toLocaleString()}</td></tr>)}
                                      </tbody>
                                  </table>
                              ...
                              <div className="flex gap-3 print:hidden">
                                  <button onClick={handlePrint} className="flex-1 py-3 bg-slate-800 text-white font-bold rounded-lg hover:bg-slate-700 transition-colors flex items-center justify-center gap-2"><Printer size={18} /> Print / Save PDF</button>
                                  <button onClick={handleMailto} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"><Mail size={18} /> Draft Email</button>
                              </div>
              ...
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                  ...
                  <div className="p-6">
                      {!poSubmitted ? (
                          ...
                          <div className="text-center py-8"><div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4"><Check size={32} /></div><h3 className="text-xl font-bold text-slate-800">PO Uploaded!</h3><p className="text-slate-500 text-sm mt-2">Our team has been notified.</p><button onClick={() => { setPoSubmitted(false); setShowPOModal(false); }} className="mt-6 text-blue-600 font-bold text-sm hover:underline">Close</button></div>
                      )}
                  </div>
              </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                  ...
                  <div className="flex border-b border-slate-200">
                      {['OFFICE', 'DRIVER', 'SHOP', 'ADMIN'].map(tab => (
                          <button key={tab} onClick={() => setLoginTab(tab as any)} className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${loginTab === tab ? 'border-blue-600 text-blue-600 bg-blue-50' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>{tab}</button>
                      ))}
                  </div>
                  <div className="p-8">
                      <div className="space-y-4">
